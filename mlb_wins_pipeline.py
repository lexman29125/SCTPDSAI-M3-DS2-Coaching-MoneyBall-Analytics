...
def run_pipeline(train_path='assets/train.csv', test_path='assets/test.csv'):
    ...
    # Define LightGBM parameters base
    base_lgb_params = {
    ...
    # Optuna hyperparameter tuning with rolling-year CV
    def objective(trial):
        param = {
            'objective': 'regression',
            'metric': 'mae',
    ...
    oof_preds_lgb = np.full(len(train_fe), np.nan)
    ...
    for train_years, val_year in splits:
        ...
        oof_preds_lgb[val_mask] = model.predict(X_val, num_iteration=model.best_iteration)
    ...
    # Select top 15 features based on importance_mean for final model
    ...
    # Prepare stacking features: OOF predictions from LightGBM, Ridge, CatBoost
    stacking_features = np.zeros((len(train_fe), 3))  # 3 models: LGB, Ridge, CatBoost

    kf = KFold(n_splits=5, shuffle=True, random_state=42)

    # OOF predictions for Ridge and CatBoost
    ...
    for train_idx, val_idx in kf.split(train_fe):
        ...
        ridge_oof[val_idx] = ridge.predict(X_val_stack)

        # CatBoost
        cat = CatBoostRegressor(verbose=0, random_seed=42)
        cat.fit(X_train_stack, y_train_stack)
        cat_oof[val_idx] = cat.predict(X_val_stack)

    stacking_features[:, 0] = oof_preds_lgb
    stacking_features[:, 1] = ridge_oof
    stacking_features[:, 2] = cat_oof

    # Meta-model training on stacking features
    meta_model = Ridge()
    meta_model.fit(stacking_features, train_fe[target_col])

    # Train final models on full training data
    print("\nTraining final LightGBM model on full train set...")
    avg_best_iter = int(np.mean(best_iterations)) if best_iterations else n_boost_round
    ...
    print("Training final Ridge model on full train set...")
    final_ridge = Ridge()
    final_ridge.fit(train_fe[top_features], train_fe[target_col])

    print("Training final CatBoost model on full train set...")
    final_cat = CatBoostRegressor(verbose=0, random_seed=42)
    final_cat.fit(train_fe[top_features], train_fe[target_col])

    # Predict test set with base models
    try:
        print("Predicting test set with base models...")
        test_preds_lgb = final_lgb_model.predict(test_fe[top_features], num_iteration=final_lgb_model.best_iteration)
        test_preds_ridge = final_ridge.predict(test_fe[top_features])
        test_preds_cat = final_cat.predict(test_fe[top_features])

        # Create stacking test features
        test_stacking_features = np.column_stack((test_preds_lgb, test_preds_ridge, test_preds_cat))

        # Meta-model prediction
        test_preds_meta = meta_model.predict(test_stacking_features)

        print(f"Number of predictions made: {len(test_preds_meta)}")
        # Round & clip to realistic bounds - adjust as dataset dictates
        test_preds_rounded = np.round(test_preds_meta).astype(int)
    ...
    # Save OOF predictions including stacking predictions
    ...
    oof_df['W_pred_lgb'] = oof_preds_lgb
    oof_df['W_pred_ridge'] = ridge_oof
    oof_df['W_pred_cat'] = cat_oof
    oof_df['W_pred_meta'] = meta_model.predict(stacking_features)
    oof_df = oof_df.dropna(subset=['W_pred_meta'])
    ...
    mae_overall = mean_absolute_error(oof_df['W_actual'], oof_df['W_pred_meta'])
    ...
    if 'yearID' in oof_df.columns:
        ...
        mae_by_decade = oof_df.groupby('decade').apply(
            lambda x: mean_absolute_error(x['W_actual'], x['W_pred_meta'])
        )
    ...
    plt.scatter(oof_df['W_actual'], oof_df['W_pred_meta'], alpha=0.5, edgecolors='k')
