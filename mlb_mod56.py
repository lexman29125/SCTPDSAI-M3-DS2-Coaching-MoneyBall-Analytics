...
from sklearn.linear_model import LinearRegression, Ridge, ElasticNet, HuberRegressor, Lasso
...
# --- Ensemble and pipeline utilities ---
...
def run_pipeline(train_path='assets/train.csv', test_path='assets/test.csv'):
    ...
    # --- Dynamic Pythagorean exponent tuning (mod_5.6 integration) ---
    def tune_pythag_exponent(df, exp_range=np.arange(1.7, 2.05, 0.05)):
        best_exp, best_mae = 2.0, float('inf')
        for exp in exp_range:
            df['pyth_exp'] = (df['R']**exp) / (df['R']**exp + df['RA']**exp)
            df['pyth_wins'] = df['pyth_exp'] * df['G']
            mae = mean_absolute_error(df['W'], df['pyth_wins'])
            if mae < best_mae:
                best_mae, best_exp = mae, exp
        print(f"Best Pythagorean exponent found: {best_exp:.3f} (MAE={best_mae:.4f})")
        return best_exp

    best_exp = tune_pythag_exponent(train)
    train['pyth_wins_tuned'] = (train['R']**best_exp) / (train['R']**best_exp + train['RA']**best_exp) * train['G']
    test['pyth_wins_tuned'] = (test['R']**best_exp) / (test['R']**best_exp + test['RA']**best_exp) * test['G']
    ...
    # --- Ensure columns needed for OBP, SLG, RC exist in both train and test ---
    # Columns to check and fill with zeros if missing
    required_zero_cols = ['HBP', 'SF', 'TB']
    ...
    # --- Sabermetric metrics (mod_5.6 integration) ---
    ...
    train['RC'] = ((train['H'] + train['BB']) * train['TB']) / (train['AB'] + train['BB'] + 1e-5)
    test['RC'] = ((test['H'] + test['BB']) * test['TB']) / (test['AB'] + test['BB'] + 1e-5)

    if 'BPF' in train.columns:
        train['OPS_BPF'] = train['OPS'] * (train['BPF'] / 100)
        test['OPS_BPF'] = test['OPS'] * (test['BPF'] / 100)
        train['run_diff_BPF'] = train['run_diff'] * (train['BPF'] / 100)
        test['run_diff_BPF'] = test['run_diff'] * (test['BPF'] / 100)
    ...
    # --- Add new sabermetric features if present in both train and test ---
    new_features = ['OBP','SLG','OPS','RC','OPS_BPF','run_diff_BPF','pyth_wins_tuned']
    ...
    # --- FULL ENSEMBLE APPROACH (mod_5.6 style) ---
    # All required models and utilities are already imported at the top of the file.
    import shutil

    # Use the same top_features for all models
    X_train_final = X_train_top.copy()
    X_test_final = X_test_top.copy()

    # List of base models
    base_models = [
        ('ridge', Ridge(alpha=1.0, random_state=0)),
        ('enet', ElasticNet(alpha=0.05, l1_ratio=0.7, random_state=0, max_iter=10000)),
        ('huber', HuberRegressor(epsilon=1.35, alpha=0.0005, max_iter=1000)),
        ('gbr', GradientBoostingRegressor(n_estimators=200, learning_rate=0.09, max_depth=3, random_state=0)),
        ('histgbr', HistGradientBoostingRegressor(max_iter=200, learning_rate=0.09, max_depth=3, random_state=0)),
        ('poly2_ridge', make_pipeline(PolynomialFeatures(degree=2, include_bias=False), Ridge(alpha=0.8, random_state=0))),
    ]

    # Seeds for seed-averaging
    seeds = [42, 7, 99]
    n_folds = 5
    oof_preds = {name: np.zeros(len(X_train_final)) for name, _ in base_models}
    test_preds = {name: np.zeros(len(X_test_final)) for name, _ in base_models}
    oof_mae = {}
    all_oof = {}

    print("\n--- Ensemble Model OOF CV (5 folds, seed averaging) ---")
    for name, model in base_models:
        oof_total = np.zeros(len(X_train_final))
        test_total = np.zeros(len(X_test_final))
        for seed in seeds:
            kf = KFold(n_splits=n_folds, shuffle=True, random_state=seed)
            oof = np.zeros(len(X_train_final))
            test_fold_preds = []
            for train_idx, val_idx in kf.split(X_train_final):
                ...
                # For pipeline models, set random_state if possible
                if hasattr(model, 'set_params'):
                    if 'random_state' in model.get_params():
                        model.set_params(random_state=seed)
                elif hasattr(model, 'steps'):
                    # For pipeline, set random_state in last step if possible
                    try:
                        if 'random_state' in model.named_steps[model.steps[-1][0]].get_params():
                            model.named_steps[model.steps[-1][0]].set_params(random_state=seed)
                    except Exception:
                        pass
                fitted = model.fit(X_tr, y_tr)
                oof[val_idx] = fitted.predict(X_val)
                test_fold_preds.append(fitted.predict(X_test_final))
            oof_total += oof / len(seeds)
            test_total += np.mean(test_fold_preds, axis=0) / len(seeds)
        oof_preds[name] = oof_total
        test_preds[name] = test_total
        oof_mae[name] = mean_absolute_error(y_train, oof_total)
        all_oof[name] = oof_total
        print(f"Model: {name:12s} | OOF MAE: {oof_mae[name]:.4f}")

    # --- Blending/Stacking ---
    oof_stack = np.column_stack([oof_preds[name] for name, _ in base_models])
    test_stack = np.column_stack([test_preds[name] for name, _ in base_models])
    ...
    mean_mae = mean_absolute_error(y_train, mean_oof)
    blend_results.append(('mean', mean_mae, mean_oof, mean_test))

    # Median
    median_oof = np.median(oof_stack, axis=1)
    median_test = np.median(test_stack, axis=1)
    median_mae = mean_absolute_error(y_train, median_oof)
    blend_results.append(('median', median_mae, median_oof, median_test))

    # Linear Regression stack
    stack_lr = LinearRegression(positive=False)
    stack_lr.fit(oof_stack, y_train)
    lr_oof = stack_lr.predict(oof_stack)
    lr_test = stack_lr.predict(test_stack)
    lr_mae = mean_absolute_error(y_train, lr_oof)
    blend_results.append(('stack_lr', lr_mae, lr_oof, lr_test))

    # Ridge stack (auto alpha tuning)
    best_ridge_alpha = None
    best_ridge_mae = float('inf')
    for alpha in [0.01, 0.1, 0.5, 1, 5, 10]:
        stack_ridge = Ridge(alpha=alpha)
        stack_ridge.fit(oof_stack, y_train)
        ridge_oof = stack_ridge.predict(oof_stack)
        ridge_mae = mean_absolute_error(y_train, ridge_oof)
        if ridge_mae < best_ridge_mae:
            best_ridge_mae = ridge_mae
            best_ridge_alpha = alpha
            best_ridge_stack = stack_ridge
            best_ridge_oof = ridge_oof
            best_ridge_test = stack_ridge.predict(test_stack)
    blend_results.append((f'stack_ridge_a{best_ridge_alpha}', best_ridge_mae, best_ridge_oof, best_ridge_test))

    # Non-negative Lasso stack
    stack_lasso = Lasso(alpha=0.01, positive=True, max_iter=10000)
    stack_lasso.fit(oof_stack, y_train)
    lasso_oof = stack_lasso.predict(oof_stack)
    lasso_test = stack_lasso.predict(test_stack)
    lasso_mae = mean_absolute_error(y_train, lasso_oof)
    blend_results.append(('stack_lasso_nonneg', lasso_mae, lasso_oof, lasso_test))

    # Select best ensemble
    best_blend = min(blend_results, key=lambda x: x[1])
    best_name, best_mae, best_oof, best_test = best_blend
    print(f"\nBest Ensemble: {best_name} | OOF MAE: {best_mae:.4f}")

    # Save stacking results
    stacking_results = pd.DataFrame({
        'ensemble': [b[0] for b in blend_results],
        'oof_mae': [b[1] for b in blend_results]
    })
    stacking_results_path = os.path.join(submission_dir, 'stacking_results.csv')
    stacking_results.to_csv(stacking_results_path, index=False)
    print(f"Saved stacking/blending results to: {stacking_results_path}")

    # Save OOF MAE by decade for best ensemble
    oof_df_ensemble = train[['yearID']].copy() if 'yearID' in train.columns else pd.DataFrame(index=train.index)
    oof_df_ensemble['W_actual'] = y_train
    oof_df_ensemble['W_pred'] = best_oof
    oof_df_ensemble = oof_df_ensemble.dropna(subset=['W_pred'])
    if 'yearID' in oof_df_ensemble.columns:
        oof_df_ensemble['decade'] = (oof_df_ensemble['yearID'] // 10) * 10
        mae_by_decade = oof_df_ensemble.groupby('decade').apply(
            lambda x: mean_absolute_error(x['W_actual'], x['W_pred'])
        ).reset_index().rename(columns={0:'mae'})
        oof_mae_by_decade_path = os.path.join(submission_dir, 'oof_mae_by_decade.csv')
        mae_by_decade.to_csv(oof_mae_by_decade_path, index=False)
        print(f"Saved OOF MAE by decade to: {oof_mae_by_decade_path}")

    # Save feature importances for Ridge and LinearRegression (safe version)
    feature_importances = []
    for name, model in base_models:
        # Initialize coefficients as NaN
        coefs = np.full(len(X_train_final.columns), np.nan)
        try:
            if hasattr(model, 'coef_'):
                coefs_tmp = np.ravel(model.coef_)
                # Ensure length matches number of features
                coefs[:len(coefs_tmp)] = coefs_tmp
            elif hasattr(model, 'named_steps') and 'ridge' in model.named_steps:
                coefs_tmp = np.ravel(model.named_steps['ridge'].coef_)
                coefs[:len(coefs_tmp)] = coefs_tmp
        except Exception:
            pass
    ...
    print(f"Saved feature importances to: {feature_importances_path}")

    # Save final predictions
    final_preds = np.round(best_test).astype(int)
    final_preds = np.clip(final_preds, 40, 120)
    final_predictions = pd.DataFrame({'ID': test_index, 'W': final_preds})
    final_predictions_path = os.path.join(submission_dir, 'final_predictions.csv')
    ...
    dest_code_path = os.path.join(submission_dir, 'mlb_mod56.py')
    try:
        shutil.copy(code_path, dest_code_path)
        print(f"Copied mlb_mod56.py to submission directory for reproducibility.")
    except Exception as e:
        print(f"Warning: Could not copy code file: {e}")

    print("\n--- Ensemble run complete ---")
    print(f"Best Ensemble: {best_name} | OOF MAE: {best_mae:.4f}")
    print(f"Seed averaging used: {seeds}")
