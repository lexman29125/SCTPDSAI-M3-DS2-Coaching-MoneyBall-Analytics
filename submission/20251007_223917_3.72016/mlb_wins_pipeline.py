...
def run_pipeline(train_path='assets/train.csv', test_path='assets/test.csv'):
    ...
    # --- Top 10 refined sabermetric feature set ---
    # 1. RPG (Runs per Game, offensive efficiency)
    # 2. OPS (On-base Plus Slugging, offensive efficiency)
    # 3. OBP (On-base Percentage, offensive efficiency)
    # 4. ERA (Earned Run Average, run prevention)
    # 5. FIP (Fielding Independent Pitching, run prevention)
    # 6. RA_per_game (Runs Allowed per Game, run prevention)
    # 7. RD_per_game (Run Differential per Game, overall differential)
    # 8. RD_adj (Era-normalized Run Differential, overall differential)
    # 9. SOA (Strikeouts Allowed, pitching control)
    # 10. BBA (Walks Allowed, pitching control)
    # 11. E (Errors, fielding)
    # 12. FP (Fielding Percentage, fielding)
    # Include lag/interaction/era features if relevant; era/decade dummies if present.

    # Ensure all features exist; compute if missing
    # OPS and OBP: if not present, try to compute from available stats
    def compute_OBP(df):
    ...
    # Compose final refined feature set
    refined_features = [
    ...
    # We'll keep: lag features for wins/run diff, era/decade dummies, and modern interactions if present
    # Add lag/interactions if in columns
    lag_interaction_cols = []
    for col in train.columns:
        if (
            ('lag' in col and col not in refined_features)
            or ('RD' in col and col not in refined_features)
            or ('Pythag' in col)
    ...
    final_features = [f for f in refined_features if f in train.columns and f in test.columns]
