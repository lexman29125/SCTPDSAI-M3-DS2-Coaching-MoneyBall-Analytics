...
def run_pipeline(train_path='assets/train.csv', test_path='assets/test.csv'):
    ...
    # Save OOF predictions for LightGBM
    ...
    # Additional: Baseline Linear Models OOF predictions and evaluation
    print("\nTraining baseline linear models with rolling-year CV...")

    # Initialize dicts to store OOF predictions for each model
    ...
    # For each fold, train models and predict on validation set
    for train_years, val_year in splits:
        train_mask = train_fe['yearID'].isin(train_years)
        val_mask = (train_fe['yearID'] == val_year)
    ...
    # Prepare DataFrames for OOF predictions
