...
def run_pipeline(train_path='assets/train.csv', test_path='assets/test.csv'):
    ...
    # Use a fixed set of top 4 features for Ridge Regression
    fixed_top4_features = [
        'Pythag_W',
        'run_diff',
        'RD_per_game',
    ...
    print(f"\nUsing fixed top 4 features for Ridge Regression: {fixed_top4_features}")

    # Ensure no NaNs in these features
    for col in fixed_top4_features:
        train_fe[col] = train_fe[col].fillna(0)
        test_fe[col] = test_fe[col].fillna(0)

    # Now restrict OOF prediction generation and final model training/prediction to these fixed top 4 features
    print("\nRe-training Ridge Regression with fixed top 4 features for OOF predictions...")
    oof_ridge_top4 = np.full(len(train_fe), np.nan)
    for train_years, val_year in splits:
        train_mask = train_fe['yearID'].isin(train_years)
        val_mask = (train_fe['yearID'] == val_year)
        X_train = train_fe.loc[train_mask, fixed_top4_features]
        y_train = train_fe.loc[train_mask, target_col]
        X_val = train_fe.loc[val_mask, fixed_top4_features]
        ridge_top4 = Ridge(random_state=42)
        ridge_top4.fit(X_train, y_train)
        oof_ridge_top4[val_mask] = ridge_top4.predict(X_val)

    # Save OOF predictions for Ridge with fixed top 4 features
    oof_ridge_top4_df = train_fe[['yearID']].copy() if 'yearID' in train_fe.columns else pd.DataFrame(index=train_fe.index)
    oof_ridge_top4_df['W_actual'] = train_fe[target_col]
    oof_ridge_top4_df['W_pred'] = oof_ridge_top4
    oof_ridge_top4_df = oof_ridge_top4_df.dropna(subset=['W_pred'])
    oof_top4_path = os.path.join(submission_dir, 'oof_predictions.csv')
    oof_ridge_top4_df.to_csv(oof_top4_path, index=False)
    print(f"Saved OOF predictions with fixed top 4 features to: {oof_top4_path}")

    # OOF evaluation: MAE overall and by era/decade (Ridge fixed top 4)
    mae_overall_top4 = mean_absolute_error(oof_ridge_top4_df['W_actual'], oof_ridge_top4_df['W_pred'])
    print(f"Overall OOF MAE with fixed top 4 features: {mae_overall_top4:.4f}")
    if 'yearID' in oof_ridge_top4_df.columns:
        oof_ridge_top4_df['decade'] = (oof_ridge_top4_df['yearID'] // 10) * 10
        mae_by_decade_top4 = oof_ridge_top4_df.groupby('decade').apply(
            lambda x: mean_absolute_error(x['W_actual'], x['W_pred'])
        )
        print("OOF MAE by decade with fixed top 4 features:")
        print(mae_by_decade_top4)

    # Plot OOF predicted vs actual wins (Ridge fixed top 4)
    plt.figure(figsize=(8, 8))
    plt.scatter(oof_ridge_top4_df['W_actual'], oof_ridge_top4_df['W_pred'], alpha=0.5, edgecolors='k')
    plt.plot([oof_ridge_top4_df['W_actual'].min(), oof_ridge_top4_df['W_actual'].max()],
             [oof_ridge_top4_df['W_actual'].min(), oof_ridge_top4_df['W_actual'].max()],
             'r--', lw=2)
    plt.xlabel('Actual Wins')
    plt.ylabel('OOF Predicted Wins')
    plt.title('OOF Predicted vs Actual Wins (Ridge - Fixed Top 4 Features)')
    plt.grid(True)
    plt.tight_layout()
    plot_top4_path = os.path.join(submission_dir, 'oof_pred_vs_actual.png')
    plt.savefig(plot_top4_path)
    plt.close()
    print(f"Saved OOF predicted vs actual plot with fixed top 4 features to: {plot_top4_path}")

    # Retrain Ridge on full train set with fixed top 4 features and predict test set
    print("\nTraining Ridge Regression on full train set with fixed top 4 features and predicting test set for submission...")
    ridge_final_top4 = Ridge(random_state=42)
    ridge_final_top4.fit(train_fe[fixed_top4_features], train_fe[target_col])
    test_preds_ridge_top4 = ridge_final_top4.predict(test_fe[fixed_top4_features])
    test_preds_rounded_top4 = np.round(test_preds_ridge_top4).astype(int)
    test_preds_rounded_top4 = np.clip(test_preds_rounded_top4, 40, 120)
    submission_top4 = pd.DataFrame({'ID': test_index, 'W': test_preds_rounded_top4})
    submission_top4_path = os.path.join(submission_dir, 'submission.csv')
    submission_top4.to_csv(submission_top4_path, index=False)
    print(f"Saved submission file with fixed top 4 features to: {submission_top4_path}")
