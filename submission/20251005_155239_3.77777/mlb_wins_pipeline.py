...
def run_pipeline(train_path='assets/train.csv', test_path='assets/test.csv'):
    ...
    # ---- Add lag features ----
    # Only create lag features if both 'teamID' and 'yearID' exist
    lag_features_created = False
    if 'teamID' in train.columns and 'yearID' in train.columns and \
       'teamID' in test.columns and 'yearID' in test.columns:
        lag_features_created = True
        for df in [train, test]:
            df['teamID_prev'] = df['teamID']
            df['yearID_prev'] = df['yearID'] - 1
        train['W_lag1'] = train.merge(
            train[['teamID', 'yearID', 'W']], left_on=['teamID_prev', 'yearID_prev'], right_on=['teamID', 'yearID'], how='left', suffixes=('', '_prev')
        )['W_prev']
        test['W_lag1'] = test.merge(
            train[['teamID', 'yearID', 'W']], left_on=['teamID_prev', 'yearID_prev'], right_on=['teamID', 'yearID'], how='left', suffixes=('', '_prev')
        )['W_prev']
        # W_lag1 missingness indicator
        train['W_lag1_missing'] = train['W_lag1'].isnull().astype(int)
        test['W_lag1_missing'] = test['W_lag1'].isnull().astype(int)
        # W_lag2_mean: mean of previous 2 seasons
        def get_lag2_mean(df, ref_df):
            l1 = df.merge(ref_df[['teamID', 'yearID', 'W']], left_on=['teamID', 'yearID'], right_on=['teamID', 'yearID'], how='left')
            l1['prev1'] = l1['yearID'] - 1
            l1['prev2'] = l1['yearID'] - 2
            prev1 = l1.merge(ref_df[['teamID', 'yearID', 'W']], left_on=['teamID', 'prev1'], right_on=['teamID', 'yearID'], how='left', suffixes=('', '_p1'))
            prev2 = l1.merge(ref_df[['teamID', 'yearID', 'W']], left_on=['teamID', 'prev2'], right_on=['teamID', 'yearID'], how='left', suffixes=('', '_p2'))
            mean_lag = pd.concat([prev1['W_p1'], prev2['W_p2']], axis=1).mean(axis=1)
            # missingness: 1 if either lag is missing
            missing = (prev1['W_p1'].isnull() | prev2['W_p2'].isnull()).astype(int)
            return mean_lag.values, missing.values
        train['W_lag2_mean'], train['W_lag2_mean_missing'] = get_lag2_mean(train, train)
        test['W_lag2_mean'], test['W_lag2_mean_missing'] = get_lag2_mean(test, train)
        # R_lag1 and RA_lag1
        train['R_lag1'] = train.merge(
            train[['teamID', 'yearID', 'R']], left_on=['teamID_prev', 'yearID_prev'], right_on=['teamID', 'yearID'], how='left', suffixes=('', '_prev')
        )['R_prev']
        test['R_lag1'] = test.merge(
            train[['teamID', 'yearID', 'R']], left_on=['teamID_prev', 'yearID_prev'], right_on=['teamID', 'yearID'], how='left', suffixes=('', '_prev')
        )['R_prev']
        train['R_lag1_missing'] = train['R_lag1'].isnull().astype(int)
        test['R_lag1_missing'] = test['R_lag1'].isnull().astype(int)
        train['RA_lag1'] = train.merge(
            train[['teamID', 'yearID', 'RA']], left_on=['teamID_prev', 'yearID_prev'], right_on=['teamID', 'yearID'], how='left', suffixes=('', '_prev')
        )['RA_prev']
        test['RA_lag1'] = test.merge(
            train[['teamID', 'yearID', 'RA']], left_on=['teamID_prev', 'yearID_prev'], right_on=['teamID', 'yearID'], how='left', suffixes=('', '_prev')
        )['RA_prev']
        train['RA_lag1_missing'] = train['RA_lag1'].isnull().astype(int)
        test['RA_lag1_missing'] = test['RA_lag1'].isnull().astype(int)
        # Clean up temp columns
        for df in [train, test]:
            df.drop(['teamID_prev', 'yearID_prev'], axis=1, inplace=True)
    else:
        lag_features_created = False
        print("Warning: 'teamID' or 'yearID' missing, skipping lag feature creation.")
    ...
    # Interactions: Pythag_W × ERA, RD_per_game × FP, RD_adj × FP, Pythag_W × FP, W_lag1 × RD_per_game, W_lag2 × Pythag_W
    for df in [train, test]:
        df['PythagW_ERA'] = df['Pythag_W'] * df['ERA']
        df['RDpg_FP'] = df['RD_per_game'] * df['FP']
        df['RDadj_FP'] = df['RD_adj'] * df['FP']
        df['PythagW_FP'] = df['Pythag_W'] * df['FP']
        if lag_features_created:
            df['Wlag1_RDpg'] = df['W_lag1'] * df['RD_per_game']
            df['Wlag2_PythagW'] = df['W_lag2_mean'] * df['Pythag_W']
        else:
            df['Wlag1_RDpg'] = np.nan
            df['Wlag2_PythagW'] = np.nan

    # Add era normalization and sabermetric features only (core features)
    ...
    # Baseline core features (12-15 classical + normalized sabermetrics)
    core_features = [
        'R', 'RA', 'ERA', 'SOA', 'HR', 'BB', 'SO', 'E', 'FP', 'RD_adj', 'RPG_norm', 'RD_eff'
    ]
    # Lag features and missingness
    if lag_features_created:
        lag_features = [
            'W_lag1', 'W_lag1_missing', 'W_lag2_mean', 'W_lag2_mean_missing',
            'R_lag1', 'R_lag1_missing', 'RA_lag1', 'RA_lag1_missing'
        ]
        interaction_features = [
            'PythagW_ERA', 'RDpg_FP', 'RDadj_FP', 'PythagW_FP', 'Wlag1_RDpg', 'Wlag2_PythagW'
        ]
    else:
        lag_features = []
        interaction_features = [
            'PythagW_ERA', 'RDpg_FP', 'RDadj_FP', 'PythagW_FP'
        ]
    # Compose final feature list
    all_features = core_features + lag_features + interaction_features
    # Filter features to those present in both train and test (robustness)
    available_features = [f for f in all_features if f in train.columns and f in test.columns]
