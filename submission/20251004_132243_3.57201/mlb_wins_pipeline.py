...
def run_pipeline(train_path='assets/train.csv', test_path='assets/test.csv'):
    ...
    # Read back feature importances and select top 5 features by absolute coefficient magnitude
    fi_df = pd.read_csv(fi_path)
    fi_df['abs_coef'] = fi_df['coefficient'].abs()
    fi_df_sorted = fi_df.sort_values('abs_coef', ascending=False)
    top5_features = fi_df_sorted['feature'].head(5).tolist()
    print(f"Top 5 features selected based on feature importances: {top5_features}")

    # Now restrict OOF prediction generation and final model training/prediction to these top 5 features
    print("\nRe-training Ridge Regression with top 5 features for OOF predictions...")
    oof_ridge_top5 = np.full(len(train_fe), np.nan)
    for train_years, val_year in splits:
        train_mask = train_fe['yearID'].isin(train_years)
        val_mask = (train_fe['yearID'] == val_year)
        X_train = train_fe.loc[train_mask, top5_features]
        y_train = train_fe.loc[train_mask, target_col]
        X_val = train_fe.loc[val_mask, top5_features]
        ridge_top5 = Ridge(random_state=42)
        ridge_top5.fit(X_train, y_train)
        oof_ridge_top5[val_mask] = ridge_top5.predict(X_val)

    # Save OOF predictions for Ridge with top 5 features
    ...
    print(f"Saved OOF predictions with top 5 features to: {oof_top5_path}")

    # OOF evaluation: MAE overall and by era/decade (Ridge top 5)
    mae_overall_top5 = mean_absolute_error(oof_ridge_top5_df['W_actual'], oof_ridge_top5_df['W_pred'])
    print(f"Overall OOF MAE with top 5 features: {mae_overall_top5:.4f}")
    if 'yearID' in oof_ridge_top5_df.columns:
        ...
        print("OOF MAE by decade with top 5 features:")
        print(mae_by_decade_top5)

    # Plot OOF predicted vs actual wins (Ridge top 5)
    plt.figure(figsize=(8, 8))
    ...
    plt.xlabel('Actual Wins')
    plt.ylabel('OOF Predicted Wins')
    plt.title('OOF Predicted vs Actual Wins (Ridge - Top 5 Features)')
    plt.grid(True)
    plt.tight_layout()
    plot_top5_path = os.path.join(submission_dir, 'oof_pred_vs_actual.png')
    plt.savefig(plot_top5_path)
    plt.close()
    print(f"Saved OOF predicted vs actual plot with top 5 features to: {plot_top5_path}")

    # Retrain Ridge on full train set with top 5 features and predict test set
    ...
    ridge_final_top5.fit(train_fe[top5_features], train_fe[target_col])
    test_preds_ridge_top5 = ridge_final_top5.predict(test_fe[top5_features])
    ...
    print(f"Saved submission file with top 5 features to: {submission_top5_path}")
