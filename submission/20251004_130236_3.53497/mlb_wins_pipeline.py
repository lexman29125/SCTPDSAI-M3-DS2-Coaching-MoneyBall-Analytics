...
def run_pipeline(train_path='assets/train.csv', test_path='assets/test.csv'):
    ...
    best_iterations = []
    fold = 0
    # Prepare OOF predictions array for LightGBM
    ...
    # Impute NaNs in top_features (again, for safety, as top_features may be a subset)
    ...
    # Ridge Regression as primary model: OOF with rolling-year CV, MAE, retrain, and test prediction
    ...
    # Optionally, run baseline linear models for comparison (not used for submission)
    print("\nTraining baseline linear models with rolling-year CV (optional, not used for submission)...")
    oof_linear = np.full(len(train_fe), np.nan)
    oof_ridge = np.full(len(train_fe), np.nan)
    oof_lasso = np.full(len(train_fe), np.nan)
    oof_elasticnet = np.full(len(train_fe), np.nan)
    for train_years, val_year in splits:
        train_mask = train_fe['yearID'].isin(train_years)
        val_mask = (train_fe['yearID'] == val_year)
        X_train = train_fe.loc[train_mask, top_features]
        ...
        oof_linear[val_mask] = lr.predict(X_val)
        # Ridge Regression (comparison)
        ridge = Ridge(random_state=42)
        ridge.fit(X_train, y_train)
        oof_ridge[val_mask] = ridge.predict(X_val)
        # Lasso Regression
        lasso = Lasso(random_state=42, max_iter=10000)
        lasso.fit(X_train, y_train)
        oof_lasso[val_mask] = lasso.predict(X_val)
        # ElasticNet Regression
        elasticnet = ElasticNet(random_state=42, max_iter=10000)
        elasticnet.fit(X_train, y_train)
        oof_elasticnet[val_mask] = elasticnet.predict(X_val)
    # Save OOF for comparison models
    oof_linear_df = train_fe[['yearID']].copy() if 'yearID' in train_fe.columns else pd.DataFrame(index=train_fe.index)
    oof_linear_df['W_actual'] = train_fe[target_col]
    oof_linear_df['W_pred'] = oof_linear
    oof_linear_df = oof_linear_df.dropna(subset=['W_pred'])
    mae_linear = mean_absolute_error(oof_linear_df['W_actual'], oof_linear_df['W_pred'])
    print(f"Linear Regression OOF MAE: {mae_linear:.4f}")
    oof_linear_path = os.path.join(submission_dir, 'oof_predictions_linear.csv')
    oof_linear_df.to_csv(oof_linear_path, index=False)
    print(f"Saved Linear Regression OOF predictions to: {oof_linear_path}")
    oof_ridge_df = train_fe[['yearID']].copy() if 'yearID' in train_fe.columns else pd.DataFrame(index=train_fe.index)
    oof_ridge_df['W_actual'] = train_fe[target_col]
    oof_ridge_df['W_pred'] = oof_ridge
    oof_ridge_df = oof_ridge_df.dropna(subset=['W_pred'])
    mae_ridge = mean_absolute_error(oof_ridge_df['W_actual'], oof_ridge_df['W_pred'])
    print(f"Ridge Regression OOF MAE: {mae_ridge:.4f}")
    oof_ridge_path = os.path.join(submission_dir, 'oof_predictions_ridge.csv')
    oof_ridge_df.to_csv(oof_ridge_path, index=False)
    print(f"Saved Ridge Regression OOF predictions to: {oof_ridge_path}")
    oof_lasso_df = train_fe[['yearID']].copy() if 'yearID' in train_fe.columns else pd.DataFrame(index=train_fe.index)
    oof_lasso_df['W_actual'] = train_fe[target_col]
    oof_lasso_df['W_pred'] = oof_lasso
    oof_lasso_df = oof_lasso_df.dropna(subset=['W_pred'])
    mae_lasso = mean_absolute_error(oof_lasso_df['W_actual'], oof_lasso_df['W_pred'])
    print(f"Lasso Regression OOF MAE: {mae_lasso:.4f}")
    oof_lasso_path = os.path.join(submission_dir, 'oof_predictions_lasso.csv')
    oof_lasso_df.to_csv(oof_lasso_path, index=False)
    print(f"Saved Lasso Regression OOF predictions to: {oof_lasso_path}")
    oof_elasticnet_df = train_fe[['yearID']].copy() if 'yearID' in train_fe.columns else pd.DataFrame(index=train_fe.index)
    oof_elasticnet_df['W_actual'] = train_fe[target_col]
    oof_elasticnet_df['W_pred'] = oof_elasticnet
    oof_elasticnet_df = oof_elasticnet_df.dropna(subset=['W_pred'])
    mae_elasticnet = mean_absolute_error(oof_elasticnet_df['W_actual'], oof_elasticnet_df['W_pred'])
    print(f"ElasticNet Regression OOF MAE: {mae_elasticnet:.4f}")
    oof_elasticnet_path = os.path.join(submission_dir, 'oof_predictions_elasticnet.csv')
    oof_elasticnet_df.to_csv(oof_elasticnet_path, index=False)
    print(f"Saved ElasticNet Regression OOF predictions to: {oof_elasticnet_path}")
