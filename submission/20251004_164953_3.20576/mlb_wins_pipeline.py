...
def run_pipeline(train_path='assets/train.csv', test_path='assets/test.csv'):
    ...
    # --- Optuna Ridge hyperparameter tuning (mean absolute error, alpha) ---
    ...
    study_ridge.optimize(ridge_objective, n_trials=25)
    best_ridge_alpha = study_ridge.best_params['alpha']
    print(f"Tuned Ridge alpha: {best_ridge_alpha}")

    # --- Optuna LightGBM hyperparameter tuning (mean absolute error) ---
    ...
    def lgb_objective(trial):
        ...
        # Manually split train/validation for Optuna trial
        X_tr, X_val, y_tr, y_val = train_test_split(X_train_scaled, y_train, test_size=0.2, random_state=42)
        model = lgb.LGBMRegressor(**params)
        # Remove early_stopping_rounds and verbose for compatibility
        model.fit(X_tr, y_tr)
    ...
    # Train Ridge on all features with tuned alpha
    ridge = Ridge(alpha=best_ridge_alpha, random_state=42)
    ridge.fit(X_train_scaled, y_train)
    ridge_coefs = np.abs(ridge.coef_)

    # Train LightGBM on all features with tuned params
    lgb_full_params = dict(best_lgb_params)
    lgb_full_params['random_state'] = 42
    lgb_full_params['n_estimators'] = 500
    lgb_model = lgb.LGBMRegressor(**lgb_full_params)
    # Fit without early stopping or verbose
    ...
    # Retrain Ridge regression on top features with tuned alpha
    ridge_top = Ridge(alpha=best_ridge_alpha, random_state=42)
    ridge_top.fit(X_train_top, y_train)

    # Generate OOF predictions using cross-validation (KFold)
    ...
    # --- Retrain LightGBM on top features with tuned params ---
    lgb_top = lgb.LGBMRegressor(**lgb_full_params)
    lgb_top.fit(X_train_top, y_train)
    lgb_preds = lgb_top.predict(X_test_top)
    lgb_preds_rounded = np.round(lgb_preds).astype(int)
    lgb_preds_rounded = np.clip(lgb_preds_rounded, 40, 120)

    # Generate OOF predictions for LightGBM
    oof_preds_lgb = np.zeros(len(X_train_top))
    for train_idx, val_idx in kf.split(X_train_top):
        X_tr, X_val = X_train_top.iloc[train_idx], X_train_top.iloc[val_idx]
        y_tr, y_val = y_train.iloc[train_idx], y_train.iloc[val_idx]
    ...
    oof_lgb_df = train[['yearID']].copy() if 'yearID' in train.columns else pd.DataFrame(index=train.index)
    oof_lgb_df['W_actual'] = y_train
    oof_lgb_df['W_pred'] = oof_preds_lgb
    oof_lgb_df = oof_lgb_df.dropna(subset=['W_pred'])
    oof_lgb_path = os.path.join(submission_dir, 'oof_predictions_lgb_top_features.csv')
    oof_lgb_df.to_csv(oof_lgb_path, index=False)
    print(f"Saved OOF predictions for LightGBM to: {oof_lgb_path}")

    # Save LightGBM submission
    submission_lgb_top = pd.DataFrame({'ID': test_index, 'W': lgb_preds_rounded})
    submission_lgb_top_path = os.path.join(submission_dir, 'submission_lgb_top_features.csv')
    submission_lgb_top.to_csv(submission_lgb_top_path, index=False)
    print(f"Saved LightGBM top features submission file to: {submission_lgb_top_path}")

    # --- Stacking Ensemble: Ridge on OOF predictions from Ridge and LGB ---
    # Prepare stacking features
    stacking_X_train = np.column_stack([oof_preds, oof_preds_lgb])
    stacking_X_test = np.column_stack([
        ridge_top.predict(X_test_top),
        lgb_top.predict(X_test_top)
    ])
    stacking_model = Ridge(alpha=1.0, random_state=42)
    stacking_model.fit(stacking_X_train, y_train)
    stacking_preds = stacking_model.predict(stacking_X_test)
